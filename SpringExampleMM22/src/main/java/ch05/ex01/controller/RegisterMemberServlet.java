...
@WebServlet("/ch05/ex01/registerMember.do")
public class RegisterMemberServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (!errorMsg.isEmpty()) {
			RequestDispatcher rd = request.getRequestDispatcher("/ch05/ex01/registerMember.jsp");
		...
		if (errorMsg.isEmpty()) {
			response.sendRedirect(request.getContextPath() + "/ch05/ex01/registerSuccess.jsp");  
			return;
		} else {
			RequestDispatcher rd = request.getRequestDispatcher("/ch05/ex01/registerMember.jsp");
			rd.forward(request, response);
